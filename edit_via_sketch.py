...
def save_init(trainer, latent, outdir, imname, colormesh=True):
    ...
    if colormesh:  # generate mesh with surface color from 3D colornet
        with torch.no_grad():
            vertices=deep_sdf.colormesh.create_mesh(
                trainer.deepsdf_net,
                trainer.colorsdf_net,
            ...
            vertices_proj=np.zeros((len(vertices),2))
            vertices_colors=np.zeros((len(vertices),3))
            """
            x proj: side view
            y proj: top view
            z proj: front view
            ...
            plt.scatter(vertices_proj[:,0],vertices_proj[:,1],c=vertices_colors)
            plt.show()
...
def edit(trainer, init_latent, source, target):
    ...
    latent, loss = trainer.step_edit_sketch(init_shape, target, mask=mask,epoch=150)
